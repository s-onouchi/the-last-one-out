...
- **着手前に決める：Havok を入れるか。** 物が落ちる・倒れる・転がる異変を作るなら Havok（`@babylonjs/havok`、インストール済み・未使用）を導入する。導入するなら非同期の初期化、iOS 16.4 未満では動かないこと、`project.yaml` の `engine.physics` 更新をあわせて扱う（Story 002 で先送りを決定）
